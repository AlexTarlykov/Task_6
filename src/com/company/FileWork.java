...
public class FileWork {
    ...
    public static Map<String, StudentMarks> dataForMyMap() throws IOException {
        Map<String, StudentMarks> dataOfFile = new SimpleHashMap<>(127);
        ...
        while (readFile.hasNextLine()) {
            ...
            if (dataOfFile.get(strings[1]) == null) {
                StudentMarks dataOfStudent = new StudentMarks(StudentMarks.MY_IMPLEMENTATION);
    ...
    public static Map<String, StudentMarks> dataForJavaMap() throws IOException {
        Map<String, StudentMarks> dataOfFile = new HashMap<>(127);
        Scanner readFile = new Scanner(data);
        while (readFile.hasNextLine()) {
            String[] strings = readFile.nextLine().split(",");
            for (int j = 0; j < 2; j++) {
            ...
            if (dataOfFile.get(strings[1]) == null) {
                StudentMarks dataOfStudent = new StudentMarks(StudentMarks.STANDARD_IMPLEMENTATION);
                dataOfStudent.put(strings[0], Integer.parseInt(strings[2]));
                dataOfFile.put(strings[1], dataOfStudent);
            } else {
                dataOfFile.get(strings[1]).put(strings[0], Integer.parseInt(strings[2]));
